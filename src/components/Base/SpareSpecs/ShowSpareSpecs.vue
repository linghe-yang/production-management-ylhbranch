<template>
  <el-card class="box-card">
    ...
    <el-table :data="showTable" stripe :border="true">
      <el-table-column prop="id" label="备品编号"> </el-table-column>
      <el-table-column prop="length" label="备品长度">
      ...
      <el-table-column prop="width" label="备品宽度">
        <template slot-scope="scope">
          <el-input
            v-if="scope.row.id == editId"
  ...
  methods: {
    searchResource() {
      ...
      let filerReasource = this.tableData.filter((item) => {
        if (item.id.includes(filterKeywords)) {
          return item;
        }
      });
    ...
    fetchData() {
      this.$API.reqGetAllSpareSpecs().then((response) => {
    ...
    delClick(row) {
      this.$confirm("此操作将永久删除该信息, 是否继续?", "提示", {
        ...
        .then(() => {
          this.$API
            .reqDelSpareSpecs(JSON.parse(JSON.stringify(row)))
            .then(() => {
              this.fetchData();
            });
        })
    ...
    saveClick(row) {
      this.$confirm("此操作将永久更改信息, 是否继续?", "提示", {
        ...
        .then(() => {
          this.$API.reqSetSpareSpecs(JSON.parse(JSON.stringify(row)))
          .then(() => {
            this.fetchData();
            this.editId = "";
          })
